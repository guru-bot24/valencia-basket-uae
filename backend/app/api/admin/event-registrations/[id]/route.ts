...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await storage.updateEventRegistrationStatus(id, status);
    if (!updated) return NextResponse.json({ error: "Event registration not found" }, { status: 404 });
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!deleted) return NextResponse.json({ error: "Event registration not found" }, { status: 404 });
